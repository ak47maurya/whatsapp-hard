...
class WhatsAppInstance {
	...
	async dataBase() {
		try {
			const { state, saveCreds } = await useMultiFileAuthState('db/' + this.key);
		...
		if (cachedResult) {
			...
			try {
				...
				if (result.exists) {
					...
					throw new Error('The number: ' + id + ' is not a valid WhatsApp number.');
				}
			} catch (error) {
				throw new Error('The number: ' + id + ' is not a valid WhatsApp number.');
			}
		...
		if (origem === 'url') {
			...
			if (parsedUrl.protocol === 'http:' || parsedUrl.protocol === 'https:') {
				...
				if (!myArray.includes(mimetype.trim())) {
					throw new Error('File ' + mimetype + ' is not allowed for ' + data.type);
				}

				origem = data.url;
			}
		} else if (origem === 'base64') {
			if (!data.filename || data.filename === '') { throw new Error('File name is required.'); }
	...
	async setStatus(status, to, type, pause = false) {
		...
		try {
			...
			throw new Error('Failed to send attendance, please check the ID and try again.')
		}
	}
	...
	async joinURL(url) {
		try {
			...
			return {
				error: true,
				message: 'Error logging in via URL, please check if the URL is still valid or if the group is an open group.',
			}
	...
	async leaveGroup(id) {
		...
		try {
			...
			return {
				error: true,
				message:
					'Error leaving the group, please check if the group still exists.',
			}
		}
	}
	...
	async getInviteCodeGroup(id) {
		...
		try {
			...
			return {
				error: true,
				message:
					'Error verifying the group, please check if the group still exists or if you are an administrator.',
			}
		}
	}
	...
	async getInstanceInviteCodeGroup(id) {
		try {
			...
			logger.error(e)
			logger.error('Error getting invite group failed')
		}
	}
	...
	async groupSettingUpdate(id, action) {
		try {
			...
			return {
				error: false,
				message:
					'Change regarding ' + action + ' completed successfully',
			}
		} catch (e) {
			//console.log(e)
			return {
				error: true,
				message:
					'Error changing ' + action + '. Please check if you have permission or if the group exists.',
			}
		}
	}
	...
	async groupUpdateSubject(id, subject) {
		...
		try {
			...
			return {
				error: true,
				message:
					'Error changing the group name, please check if you are an administrator or if the group exists',
			}
		}
	}
	...
	async groupUpdateDescription(id, description) {
		...
		try {
			...
			return {
				error: false,
				message:
					'Description of the group changed to ' + description
			}
		} catch (e) {
			...
			return {
				error: true,
				message:
					'Error changing the group description, please check if you are an administrator or if the group exists',
			}
		}
	}
	...
	async groupGetInviteInfo(url) {
		try {
			...
			return {
				error: true,
				message:
					'Failed to fetch group information. Please check the URL code or if the group still exists.',
			}
		}
	}
	...
	async groupAcceptInvite(id) {
		try {
			...
			return {
				error: true,
				message:
					'Failed to accept group invite. Please check the invite code or if the group still exists.',
			}
		}
	}
